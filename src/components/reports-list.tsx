      ...
      <div style={{ maxWidth:1100, margin:"0 auto", padding:"1.5rem 1rem" }}>
        ...
        <div style={{ display:"flex", gap:10, marginBottom:"1rem", flexWrap:"wrap" }}>
          <input placeholder="Search by vessel or IMO…" value={search} onChange={e=>setSearch(e.target.value)}
            style={{ flex:1, minWidth:200, padding:"9px 14px", border:"1px solid #D1D5DB", borderRadius:8, fontSize:13, background:"#fff" }} />
                ...
                {filtered.map((r,i)=>(
                  <tr key={r.id} style={{ borderTop:"1px solid #F3F4F6", background:i%2===0?"#fff":"#FAFAFA" }}>
                    <td style={{ padding:"9px 12px" }}>
                      ...
                      <div style={{ fontSize:11, color:"#9CA3AF" }}>{r.imo_number??''}</div>
                    </td>
                    ...
                    <td style={{ padding:"9px 12px", textAlign:"center" }}>
                      {r.deficiencies>0 ? <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:600, background:"#FEE2E2", color:"#DC2626" }}>{r.deficiencies}</span> : "0"}
                    </td>
                    <td style={{ padding:"9px 12px" }}>
                      <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:500, background:STATUS_BG[r.status??"DRAFT"], color:STATUS_COLOR[r.status??"DRAFT"] }}>
                        {(r.status??"DRAFT").replace("_"," ")}
                      </span>
                    </td>
                    <td style={{ padding:"9px 12px", whiteSpace:"nowrap", display:"flex", gap:4 }}>
                      <Link href={`/inspections/${r.id}`} style={{ padding:"3px 10px", background:TEAL, color:"#fff", borderRadius:5, fontSize:12, textDecoration:"none" }}>View</Link>
                      {r.vessel_id && <Link href={`/vessels/${r.vessel_id}`} style={{ padding:"3px 8px", background:"#F3F4F6", color:"#374151", borderRadius:5, fontSize:12, textDecoration:"none" }}>History</Link>}
                    </td>
                  </tr>
                ))}
              </tbody>
