...
interface Stats { vessels:number; inspections:number; openDeficiencies:number; thisMonth:number; }
interface RecentInspection { id:string; vessel_name:string; vessel_type:string; inspection_type:string; status:string; inspector_name:string; started_at:string; overall_grade:string; created_at:string; }
interface Deficiency { id:string; prompt:string; section_code:string; grade_value:string; deficiency_status:string; deficiency_action:string; remarks:string; vessel_name:string; vessel_type:string; started_at:string; inspection_type:string; }
...
export default function DashboardOverview({ stats, recentInspections, deficiencies }:{stats:Stats;recentInspections:RecentInspection[];deficiencies:Deficiency[]}) {
  ...
  async function updateStatus(id:string, status:string, action?:string) {
    await fetch(`/api/deficiencies/${id}`, { method:"PATCH", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ deficiency_status:status, deficiency_action:action??"" }) });
    setDefs(prev => prev.map(d => d.id===id ? {...d,deficiency_status:status,deficiency_action:action??d.deficiency_action} : d));
    setActionId(null); setActionText("");
  }

  const statCards = [
    { label:"Vessels in fleet",     value:stats.vessels,          icon:"🚢", accent:TEAL,     href:"/vessels" },
    { label:"Total inspections",    value:stats.inspections,      icon:"📋", accent:"#2563EB", href:"/reports" },
    { label:"Open deficiencies",    value:stats.openDeficiencies, icon:"⚠️", accent:"#DC2626", href:null },
    { label:"Inspections this month",value:stats.thisMonth,       icon:"📅", accent:"#059669", href:"/reports" },
  ];
    ...
    <div style={{ fontFamily:"system-ui,-apple-system,sans-serif", background:"#F4F2EE", minHeight:"100vh" }}>
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"1.5rem 1rem" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.5rem" }}>
          ...
          <Link href="/inspections/new" style={{ background:NAV, color:"#fff", padding:"8px 18px", borderRadius:7, fontSize:13, fontWeight:500, textDecoration:"none" }}>
            + New Inspection
          </Link>
        </div>

        {/* Stat cards — clickable where relevant */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:12, marginBottom:"1.5rem" }}>
          {statCards.map(c => {
            const inner = (
              <div style={{ background:"#fff", borderRadius:10, padding:"1.1rem 1.25rem", border:"1px solid #E5E7EB", cursor:c.href?"pointer":"default", transition:"box-shadow 0.15s" }}
                onMouseOver={e=>{if(c.href)(e.currentTarget as HTMLDivElement).style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"}}
                ...
                {c.href && <div style={{ fontSize:11, color:TEAL, marginTop:4 }}>View →</div>}
              </div>
            );
            return c.href
        ...
        <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", overflow:"hidden" }}>
          <div style={{ padding:"1rem 1.25rem", borderBottom:"1px solid #F3F4F6" }}>
            ...
            <div style={{ display:"flex", gap:4 }}>
              {(["open","in_progress","closed","all"] as const).map(tab=>(
                <button key={tab} onClick={()=>setActiveTab(tab)} style={{ padding:"4px 12px", borderRadius:20, fontSize:12, border:"none", cursor:"pointer",
                  fontWeight:activeTab===tab?600:400,
                  background:activeTab===tab?NAV:"#F3F4F6",
                  color:activeTab===tab?"#fff":"#6B7280" }}>
                  {tab==="in_progress"?"In Progress":tab.charAt(0).toUpperCase()+tab.slice(1)}
                  {tab!=="all" && <span style={{ marginLeft:5, opacity:0.75 }}>
                    ({defs.filter(d=>tab==="open"?(!d.deficiency_status||d.deficiency_status==="OPEN"):d.deficiency_status===tab.toUpperCase()).length})
                  </span>}
                </button>
              ))}
            </div>
          ...
          {visibleDefs.length===0 ? (
            <div style={{ padding:"2.5rem", textAlign:"center", color:"#9CA3AF", fontSize:13 }}>
              {activeTab==="open"?"No open deficiencies — all clear! ✅":"No deficiencies in this category."}
            </div>
          ) : (
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  ...
                  {visibleDefs.map((d,i)=>(
                    <tr key={d.id} style={{ borderTop:"1px solid #F3F4F6", background:i%2===0?"#fff":"#FAFAFA", verticalAlign:"top" }}>
                      ...
                      <td style={{ padding:"9px 12px", color:"#6B7280", whiteSpace:"nowrap" }}>
                        ...
                        <div style={{ fontSize:11, color:"#9CA3AF" }}>{TYPES[d.vessel_type]??d.vessel_type}</div>
                      </td>
                      <td style={{ padding:"9px 12px", color:"#6B7280", fontSize:11 }}>{d.section_code}</td>
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:600, ...Object.fromEntries((GRADE_STYLE[d.grade_value]??"").split(";").filter(Boolean).map(s=>s.split(":"))) }}>
                          {d.grade_value==="ACTION_REQUIRED"?"Action Req.":d.grade_value}
                        </span>
                      </td>
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:500, ...Object.fromEntries((STATUS_STYLE[d.deficiency_status??"OPEN"]??STATUS_STYLE.OPEN).split(";").filter(Boolean).map(s=>s.split(":"))) }}>
                          {STATUS_LABEL[d.deficiency_status??"OPEN"]??"Open"}
                        </span>
                      </td>
                      <td style={{ padding:"9px 12px", maxWidth:220, color:"#6B7280", fontSize:12 }}>
                        {actionId===d.id ? (
                          <input value={actionText} onChange={e=>setActionText(e.target.value)} placeholder="Describe corrective action…"
                      ...
                      <td style={{ padding:"9px 12px", whiteSpace:"nowrap" }}>
                        {actionId===d.id ? (
                          <div style={{ display:"flex", gap:4, flexDirection:"column" }}>
                            <button onClick={()=>updateStatus(d.id,"IN_PROGRESS",actionText)} style={{ padding:"3px 8px", background:"#FEF3C7", color:"#92400E", border:"none", borderRadius:5, fontSize:11, cursor:"pointer", fontWeight:500 }}>In Progress</button>
                            <button onClick={()=>updateStatus(d.id,"CLOSED",actionText)} style={{ padding:"3px 8px", background:"#D1FAE5", color:"#065F46", border:"none", borderRadius:5, fontSize:11, cursor:"pointer", fontWeight:500 }}>Close</button>
                            <button onClick={()=>setActionId(null)} style={{ padding:"3px 8px", background:"#F3F4F6", color:"#6B7280", border:"none", borderRadius:5, fontSize:11, cursor:"pointer" }}>Cancel</button>
                          </div>
                        ) : (
