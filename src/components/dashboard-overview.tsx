    ...
    <div style={{ fontFamily:"system-ui,-apple-system,sans-serif", background:"#F4F2EE", minHeight:"100vh" }}>
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"1.5rem 1rem" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.5rem" }}>
          <div>
            <h1 style={{ fontSize:23, fontWeight:600, color:"#1A2533", margin:0 }}>Dashboard</h1>
            <p style={{ fontSize:14, color:"#6B7280", marginTop:2 }}>Fleet condition overview and deficiency tracking</p>
          </div>
          <Link href="/inspections/new" style={{
            background: NAV, color:"#fff", padding:"8px 18px",
            borderRadius:7, fontSize:15, fontWeight:500,
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:12, marginBottom:"1.5rem" }}>
          {statCards.map(c => (
            <div key={c.label} style={{
              ...
              <div style={{ fontSize:25, marginBottom:6 }}>{c.icon}</div>
              <div style={{ fontSize:32, fontWeight:700, color: c.accent, lineHeight:1 }}>{c.value}</div>
              <div style={{ fontSize:14, color:"#6B7280", marginTop:5 }}>{c.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", marginBottom:"1.5rem", overflow:"hidden" }}>
          <div style={{ padding:"1rem 1.25rem", borderBottom:"1px solid #F3F4F6", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <h2 style={{ fontSize:16, fontWeight:600, color:"#1A2533", margin:0 }}>Recent Inspections</h2>
            <Link href="/inspections/new" style={{ fontSize:14, color:TEAL, textDecoration:"none" }}>Start new →</Link>
          </div>
          {recentInspections.length === 0 ? (
            <div style={{ padding:"2.5rem", textAlign:"center", color:"#9CA3AF", fontSize:15 }}>
                ...
                <thead>
                  <tr style={{ background:"#F9FAFB" }}>
                    {["Vessel","Type","Inspection","Inspector","Date","Status","Grade"].map(h => (
                      <th key={h} style={{ padding:"8px 12px", textAlign:"left", fontWeight:500, color:"#6B7280", fontSize:13, textTransform:"uppercase", letterSpacing:"0.05em", whiteSpace:"nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {recentInspections.map((r, i) => (
                    <tr key={r.id} style={{ borderTop:"1px solid #F3F4F6", background: i%2===0?"#fff":"#FAFAFA" }}>
                      ...
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:13, fontWeight:500, background:"#E0F2FE", color:"#0369A1" }}>
                          {r.status ?? "Draft"}
                        </span>
                      </td>
        ...
        <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", overflow:"hidden" }}>
          <div style={{ padding:"1rem 1.25rem", borderBottom:"1px solid #F3F4F6" }}>
            <h2 style={{ fontSize:16, fontWeight:600, color:"#1A2533", margin:0, marginBottom:10 }}>Deficiency Tracker</h2>
            {/* Filter tabs */}
            <div style={{ display:"flex", gap:4 }}>
              {(["open","in_progress","closed","all"] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{
                  padding:"4px 12px", borderRadius:20, fontSize:14, border:"none", cursor:"pointer",
                  ...
                  {visibleDefs.map((d, i) => (
                    <tr key={d.id} style={{ borderTop:"1px solid #F3F4F6", background: i%2===0?"#fff":"#FAFAFA", verticalAlign:"top" }}>
                      <td style={{ padding:"9px 12px", maxWidth:280 }}>
                        ...
                        <div style={{ fontSize:13, color:"#9CA3AF", marginTop:2 }}>{fmt(d.started_at)}</div>
                      </td>
                      <td style={{ padding:"9px 12px", color:"#6B7280", whiteSpace:"nowrap" }}>
                        <div style={{ fontWeight:500, color:"#374151" }}>{d.vessel_name}</div>
                        <div style={{ fontSize:13, color:"#9CA3AF" }}>{TYPE_LABEL[d.vessel_type] ?? d.vessel_type}</div>
                      </td>
                      <td style={{ padding:"9px 12px", color:"#6B7280", fontSize:13 }}>{d.section_code}</td>
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:13, fontWeight:600, ...(Object.fromEntries(GRADE_STYLE[d.grade_value]?.split(";").map(s => s.split(":")) ?? [])) }}>
                          {d.grade_value === "ACTION_REQUIRED" ? "Action Req." : d.grade_value}
                        </span>
                      </td>
                      <td style={{ padding:"9px 12px" }}>
                        <span style={{ padding:"2px 8px", borderRadius:20, fontSize:13, fontWeight:500, ...(Object.fromEntries((STATUS_STYLE[d.deficiency_status ?? "OPEN"] ?? STATUS_STYLE.OPEN).split(";").map(s => s.split(":")))) }}>
                          {STATUS_LABEL[d.deficiency_status ?? "OPEN"] ?? "Open"}
                        </span>
                      </td>
                      <td style={{ padding:"9px 12px", maxWidth:220, color:"#6B7280", fontSize:14 }}>
                        {actionId === d.id ? (
                          <input value={actionText} onChange={e => setActionText(e.target.value)}
                            placeholder="Describe corrective action…"
                            style={{ width:"100%", padding:"4px 8px", border:"1px solid #D1D5DB", borderRadius:5, fontSize:14 }} />
                        ) : (
                        ...
                        {actionId === d.id ? (
                          <div style={{ display:"flex", gap:4, flexDirection:"column" }}>
                            <button onClick={() => updateStatus(d.id, "IN_PROGRESS", actionText)}
                              style={{ padding:"3px 8px", background:"#FEF3C7", color:"#92400E", border:"none", borderRadius:5, fontSize:13, cursor:"pointer", fontWeight:500 }}>
                              In Progress
                            </button>
                            <button onClick={() => updateStatus(d.id, "CLOSED", actionText)}
                              style={{ padding:"3px 8px", background:"#D1FAE5", color:"#065F46", border:"none", borderRadius:5, fontSize:13, cursor:"pointer", fontWeight:500 }}>
                              Close
                            </button>
                            <button onClick={() => setActionId(null)}
                              style={{ padding:"3px 8px", background:"#F3F4F6", color:"#6B7280", border:"none", borderRadius:5, fontSize:13, cursor:"pointer" }}>
                              Cancel
                            </button>
                          </div>
                        ) : (
                          <button onClick={() => { setActionId(d.id); setActionText(d.deficiency_action ?? ""); }}
                            style={{ padding:"4px 10px", background: NAV, color:"#fff", border:"none", borderRadius:5, fontSize:13, cursor:"pointer", whiteSpace:"nowrap" }}>
                            Update →
                          </button>
                        )}
                      </td>
