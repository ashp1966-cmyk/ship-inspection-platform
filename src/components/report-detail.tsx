  ...
  return (
    <div style={{ fontFamily:"system-ui,sans-serif", background:"#F4F2EE", minHeight:"100vh" }}>
      ...
      <div className="no-print" style={{ background:"#0A1628", padding:"10px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <a href="/reports" style={{ color:"#4ABFDA", fontSize:14, textDecoration:"none" }}>← Back to reports</a>
        <button onClick={() => window.print()} style={{ padding:"6px 16px", background:"#1BA5C0", color:"#fff", border:"none", borderRadius:6, fontSize:14, cursor:"pointer", fontWeight:500 }}>
          🖨 Print / Export PDF
        </button>
      </div>
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"1.5rem 1rem" }}>
        ...
        {deficiencies.length > 0 && (
          <div style={{ background:"#FEF2F2", borderRadius:10, border:"1px solid #FECACA", padding:"1rem 1.5rem", marginBottom:"1rem" }}>
            <h2 style={{ fontSize:16, fontWeight:600, color:"#DC2626", marginBottom:10 }}>⚠ Deficiencies ({deficiencies.length})</h2>
            {deficiencies.map((d,i) => (
              <div key={d.id} style={{ display:"flex", gap:10, marginBottom:6, fontSize:15 }}>
                ...
                <span style={{ padding:"1px 6px", borderRadius:12, fontSize:13, fontWeight:600, background: GRADE_BG[d.grade_value], color: GRADE_COLOR[d.grade_value] }}>
                  {d.grade_value?.replace("_"," ")}
                </span>
                {d.deficiency_status && d.deficiency_status!=="OPEN" && (
                  <span style={{ padding:"1px 6px", borderRadius:12, fontSize:13, background:"#D1FAE5", color:"#065F46" }}>{d.deficiency_status}</span>
                )}
              </div>
            ))}
          </div>
        ...
        {Object.entries(sections).map(([code, sItems]) => (
          <div key={code} style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", marginBottom:"1rem", overflow:"hidden" }}>
            <div style={{ padding:"10px 16px", background:"#F9FAFB", borderBottom:"1px solid #E5E7EB" }}>
              <h3 style={{ fontSize:15, fontWeight:600, color:"#1A2533", margin:0 }}>{code.replace(/_/g," ")}</h3>
            </div>
            <div>
              {sItems.map((item, i) => {
                ...
                return (
                  <div key={item.id} style={{ padding:"8px 16px", borderBottom:"1px solid #F3F4F6", background: isDeficiency?"#FFF9F9":"transparent", display:"flex", alignItems:"flex-start", gap:12 }}>
                    <span style={{ fontSize:13, color:"#D1D5DB", minWidth:22, paddingTop:1 }}>{i+1}.</span>
                    <span style={{ flex:1, fontSize:15, color:"#374151", lineHeight:1.5 }}>{item.prompt ?? item.equipment_name}</span>
                    {answer && (
                      <span style={{ padding:"2px 8px", borderRadius:20, fontSize:13, fontWeight:500, whiteSpace:"nowrap",
                        background: GRADE_BG[answer] ?? "#F3F4F6",
                        color: GRADE_COLOR[answer] ?? "#374151" }}>
                        {String(answer).replace("_"," ")}
                      </span>
                    )}
                    {item.remarks && <span style={{ fontSize:13, color:"#6B7280", fontStyle:"italic", maxWidth:200 }}>{item.remarks}</span>}
                    {/* Photos */}
