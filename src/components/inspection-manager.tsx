...
function ScoreDial({ score, label }: { score: number; label: string }) {
  ...
  return (
    <div style={{ textAlign:"center", padding:"12px 20px" }}>
      <div style={{ fontSize:36, fontWeight:700, color: GRADE_COLOR[grade] }}>{score}</div>
      <div style={{ fontSize:10, color:"#9CA3AF", textTransform:"uppercase", letterSpacing:"0.06em" }}>/100</div>
      <div style={{ fontSize:12, color:"#6B7280", marginTop:4 }}>{label}</div>
      <span style={{ padding:"2px 8px", borderRadius:20, fontSize:11, fontWeight:600, background:GRADE_BG[grade], color:GRADE_COLOR[grade] }}>
        {grade.replace("_"," ")}
      </span>
    </div>
  );
  ...
  return (
    <div style={{ fontFamily:"system-ui,sans-serif", background:"#F4F2EE", minHeight:"100vh" }}>
      ...
      <div className="no-print" style={{ background:"#0A1628", padding:"10px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <Link href="/reports" style={{ color:"#4ABFDA", fontSize:12, textDecoration:"none" }}>← Reports</Link>
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={() => window.print()} style={{ padding:"6px 14px", background:"rgba(255,255,255,0.1)", color:"#fff", border:"1px solid rgba(255,255,255,0.2)", borderRadius:6, fontSize:12, cursor:"pointer" }}>
            🖨 Print / PDF
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"1.5rem 1rem" }}>
        ...
        <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", padding:"1.25rem", marginBottom:"1rem" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:"1rem" }}>
            <div>
              <h1 style={{ fontSize:20, fontWeight:700, color:"#1A2533", margin:0 }}>{insp.vessel_name ?? "Unknown Vessel"}</h1>
            ...
            <span style={{ padding:"4px 14px", borderRadius:20, fontSize:12, fontWeight:600, background: STATUS_BG[insp.status??"DRAFT"], color:STATUS_COLOR[insp.status??"DRAFT"] }}>
              {insp.status ?? "DRAFT"}
            </span>
          </div>
              ...
              <div key={s} style={{ display:"flex", alignItems:"center" }}>
                <div style={{ textAlign:"center" }}>
                  <div style={{ width:28, height:28, borderRadius:"50%", border:`2px solid ${i<=currentIdx?STATUS_COLOR[s]:"#D1D5DB"}`,
                    ...
                    display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700, margin:"0 auto" }}>
                    {i < currentIdx ? "✓" : i+1}
                  </div>
                  <div style={{ fontSize:10, color: i<=currentIdx?STATUS_COLOR[s]:"#9CA3AF", marginTop:3, whiteSpace:"nowrap" }}>{s.replace("_"," ")}</div>
                </div>
          ...
          {!isIssued && (
            <div style={{ marginTop:"1rem", display:"flex", gap:10, flexWrap:"wrap" }}>
              <input placeholder="Port" value={port} onChange={e=>setPort(e.target.value)}
                style={{ padding:"6px 10px", border:"1px solid #D1D5DB", borderRadius:6, fontSize:13, width:150 }} />
              ...
              <button onClick={advanceStatus} disabled={updating}
                style={{ padding:"6px 16px", background:STATUS_COLOR[STATUS_FLOW[currentIdx+1]??"ISSUED"]??"#059669",
                  color:"#fff", border:"none", borderRadius:6, fontSize:12, cursor:"pointer", fontWeight:500 }}>
                {updating?"Updating…":`→ Mark as ${STATUS_FLOW[currentIdx+1]?.replace("_"," ")??""}`}
              </button>
              <button onClick={calculateScore} disabled={scoring}
                style={{ padding:"6px 16px", background:"#0A1628", color:"#fff", border:"none", borderRadius:6, fontSize:12, cursor:"pointer", fontWeight:500 }}>
        ...
        {sects.length > 0 && (
          <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", marginBottom:"1rem", overflow:"hidden" }}>
            <div style={{ padding:"10px 16px", background:"#F9FAFB", borderBottom:"1px solid #E5E7EB" }}>
              <h2 style={{ fontSize:13, fontWeight:600, margin:0 }}>Section Scores</h2>
            </div>
              ...
              {sects.filter(s=>s.graded_items>0).sort((a:any,b:any)=>a.score-b.score).map((s:any) => {
                ...
                return (
                  <div key={s.section_code} style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:11, color:"#374151", marginBottom:3 }}>{s.section_title || s.section_code.replace(/_/g," ")}</div>
                      <div style={{ height:6, background:"#F3F4F6", borderRadius:3, overflow:"hidden" }}>
                        <div style={{ height:"100%", width:`${s.score}%`, background:GRADE_COLOR[grade], borderRadius:3, transition:"width 0.5s" }} />
                      </div>
                    </div>
                    <span style={{ fontSize:12, fontWeight:600, color:GRADE_COLOR[grade], minWidth:28, textAlign:"right" }}>{s.score}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", padding:"1.25rem", marginBottom:"1rem" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            <h2 style={{ fontSize:13, fontWeight:600, margin:0 }}>Executive Summary</h2>
            {!editSummary && !isIssued && (
              <button onClick={()=>setEditSummary(true)} style={{ fontSize:12, color:"#1BA5C0", border:"none", background:"none", cursor:"pointer" }}>Edit</button>
            )}
          </div>
          {editSummary ? (
            <div>
              <textarea value={summary} onChange={e=>setSummary(e.target.value)} rows={6}
                style={{ width:"100%", padding:"8px", border:"1px solid #D1D5DB", borderRadius:6, fontSize:13, resize:"vertical", fontFamily:"inherit" }} />
              <div style={{ display:"flex", gap:8, marginTop:8 }}>
                <button onClick={saveSummary} style={{ padding:"6px 14px", background:"#0A1628", color:"#fff", border:"none", borderRadius:6, fontSize:12, cursor:"pointer" }}>Save</button>
                <button onClick={()=>{setEditSummary(false);setSummary(insp.executive_summary??'');}} style={{ padding:"6px 12px", background:"#F3F4F6", border:"none", borderRadius:6, fontSize:12, cursor:"pointer" }}>Cancel</button>
              </div>
            </div>
          ) : (
        ...
        {deficiencies.length > 0 && (
          <div style={{ background:"#FEF2F2", borderRadius:10, border:"1px solid #FECACA", padding:"1.25rem", marginBottom:"1rem" }}>
            <h2 style={{ fontSize:13, fontWeight:600, color:"#DC2626", marginBottom:10 }}>⚠ Deficiencies ({deficiencies.length})</h2>
            {deficiencies.map((d,i)=>(
              <div key={d.id} style={{ display:"flex", gap:10, marginBottom:6, fontSize:13 }}>
                ...
                <span style={{ padding:"1px 8px", borderRadius:20, fontSize:11, fontWeight:600, background:GRADE_BG[d.grade_value]??"#FEE2E2", color:GRADE_COLOR[d.grade_value]??"#DC2626", whiteSpace:"nowrap" }}>
        ...
        {Object.entries(grouped).map(([code,sItems])=>(
          <div key={code} style={{ background:"#fff", borderRadius:10, border:"1px solid #E5E7EB", marginBottom:"0.75rem", overflow:"hidden" }}>
            <div style={{ padding:"8px 16px", background:"#F9FAFB", borderBottom:"1px solid #E5E7EB", display:"flex", justifyContent:"space-between" }}>
              <h3 style={{ fontSize:12, fontWeight:600, color:"#374151", margin:0 }}>{code.replace(/_/g," ")}</h3>
              <span style={{ fontSize:11, color:"#9CA3AF" }}>{sItems.length} items</span>
            </div>
            {sItems.map((item,i)=>{
              ...
              return (
                <div key={item.id} style={{ padding:"7px 16px", borderBottom:"1px solid #F3F4F6", background:isDeficiency?"#FFF9F9":"transparent", display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:11, color:"#D1D5DB", minWidth:20 }}>{i+1}.</span>
                  <span style={{ flex:1, fontSize:12, color:"#374151" }}>{item.prompt||item.equipment_name}</span>
                  {answer && (
                    <span style={{ padding:"1px 7px", borderRadius:20, fontSize:11, fontWeight:500, whiteSpace:"nowrap",
                      background: GRADE_BG[answer]??"#F3F4F6", color: GRADE_COLOR[answer]??"#374151" }}>
                      {String(answer).replace("_"," ")}
                    </span>
                  )}
                  {item.remarks && <span style={{ fontSize:11, color:"#6B7280", fontStyle:"italic", maxWidth:180, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }} title={item.remarks}>{item.remarks}</span>}
