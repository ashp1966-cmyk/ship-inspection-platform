    ...
    <div style={{ fontFamily:"system-ui,sans-serif", background:"#F4F2EE", minHeight:"100vh" }}>
      <div style={{ maxWidth:900, margin:"0 auto", padding:"1.5rem 1rem" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.25rem" }}>
          <div>
            <h1 style={{ fontSize:23, fontWeight:600, color:"#1A2533", margin:0 }}>User Management</h1>
            <p style={{ fontSize:14, color:"#6B7280", marginTop:2 }}>{users.length} user{users.length!==1?"s":""} · Roles: admin, inspector, viewer</p>
          </div>
          <button onClick={openAdd} style={{ background:NAV, color:"#fff", padding:"8px 18px", borderRadius:7, fontSize:15, fontWeight:500, border:"none", cursor:"pointer" }}>+ Add User</button>
        </div>
              ...
              <thead>
                <tr style={{ background:"#F9FAFB" }}>
                  {["Name","Email","Role","Status","Added","Actions"].map(h => (
                    <th key={h} style={{ padding:"9px 12px", textAlign:"left", fontWeight:500, color:"#6B7280", fontSize:13, textTransform:"uppercase", letterSpacing:"0.04em" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map((u, i) => (
                  <tr key={u.id} style={{ borderTop:"1px solid #F3F4F6", background:i%2===0?"#fff":"#FAFAFA" }}>
                    ...
                    <td style={{ padding:"9px 12px" }}>
                      <span style={{ padding:"2px 10px", borderRadius:20, fontSize:13, fontWeight:600,
                    ...
                    <td style={{ padding:"9px 12px", whiteSpace:"nowrap" }}>
                      <button onClick={() => openEdit(u)} style={{ padding:"3px 10px", background:"#F3F4F6", border:"none", borderRadius:5, fontSize:14, cursor:"pointer", marginRight:6 }}>Edit</button>
                      <button onClick={() => deleteUser(u.id)} style={{ padding:"3px 8px", background:"#FEE2E2", color:"#DC2626", border:"none", borderRadius:5, fontSize:14, cursor:"pointer" }}>✕</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50, padding:"1rem" }}>
          <div style={{ background:"#fff", borderRadius:12, width:"100%", maxWidth:440, padding:"1.5rem" }}>
            <h2 style={{ fontSize:18, fontWeight:600, marginBottom:"1.25rem" }}>{editing ? "Edit User" : "Add User"}</h2>
            {[
              ...
              <div key={f.key} style={{ marginBottom:10 }}>
                <label style={{ fontSize:13, fontWeight:500, color:"#6B7280", textTransform:"uppercase", letterSpacing:"0.05em", display:"block", marginBottom:3 }}>{f.label}</label>
                <input type={f.type??"text"} value={(form as any)[f.key]} disabled={f.disabled}
                  onChange={e => setForm(prev => ({...prev,[f.key]:e.target.value}))}
                  style={{ width:"100%", padding:"7px 10px", border:"1px solid #D1D5DB", borderRadius:6, fontSize:15, background:f.disabled?"#F9FAFB":"#fff" }} />
              </div>
            ))}
            <div style={{ marginBottom:10 }}>
              <label style={{ fontSize:13, fontWeight:500, color:"#6B7280", textTransform:"uppercase", letterSpacing:"0.05em", display:"block", marginBottom:3 }}>Role</label>
            ...
            {editing && (
              <label style={{ display:"flex", alignItems:"center", gap:8, fontSize:15, marginBottom:14, cursor:"pointer" }}>
