...
export function getPrePurchaseSections(vesselType: VesselType): Section[] {
  return [
    ...
    ...universalPrePurchase,
    ...typeSpecificPrePurchase.filter(s => s.vesselType === vesselType),
  ];
}
...
const universalInventory: EquipmentItem[] = [
  ...baseInventory("MACHINERY_INV",["Main Engine","Auxiliary Engine No.1","Auxiliary Engine No.2","Auxiliary Engine No.3","Boiler (Composite/Steam)","Steering Gear","Fresh Water Generator","Air Compressor No.1","Air Compressor No.2","Oily Water Separator","Sewage Treatment Plant","Ballast Water Treatment System","Incinerator","ME Flow Meter","MGPS (Marine Growth Prevention System)"]),
  ...baseInventory("NAV_COMMS_INV",["Radar (X-Band)","Radar (S-Band)","ECDIS No.1","ECDIS No.2","Echo Sounder","GPS / DGPS","AIS Transponder","GMDSS Console","VDR","BNWAS","EPIRB","SART","Aldis Lamp","Navtex Receiver","Gyro Compass"]),
  ...baseInventory("SAFETY_INV",["Lifeboat(s)","Rescue Boat","Liferafts","SCBA Sets","EEBD Sets","Fixed CO2 System","Fixed Foam System","Main Fire Pump","Emergency Fire Pump","Portable Gas Detectors (Multi-gas)","Portable Gas Detectors (Drager)"]),
];
...
const typeInventory: Record<VesselType, EquipmentItem[]> = {
  ...
  LNG_CARRIER: baseInventory("GAS_PLANT_INV",["Cargo Compressors","Reliquefaction Plant","Gas Detection System (Fixed)","ESD System","Cargo Heaters/Vaporisers"]),
